...
class iphonebuy extends StatefulWidget {
  iphonebuy({Key? key, required this.index, required this.routeargs1})
      : super(key: key);
  int index;
  int routeargs1;
  ...
  @override
  State<iphonebuy> createState() => _iphonebuyState();
}
...
Future<List<Map>> getdata(String name, String imageurl, String desc,
    String price, index, int quntity,String email) async {
...
class _iphonebuyState extends State<iphonebuy> {
  @override
  Widget build(BuildContext context) {
    final iphone_product = Provider.of<data_provider>(context);
    final object = iphone_product.iphoneitems();
    return SafeArea(
        child: Scaffold(
            body: Column(
      children: [
        iimage(imageurl2: object[widget.routeargs1][widget.index].imageurl),
        Text(
          object[widget.routeargs1][widget.index].title,
          style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold),
        ),
        idesc(title2: object[widget.routeargs1][widget.index].desc),
        SizedBox(
          height: 20,
        ),
        ibuttons(
          index: widget.index,
          routeargs: widget.routeargs1,
        )
      ],
    )));
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
          image: DecorationImage(image: NetworkImage("${imageurl2}"))),
    );
  }
}
      ...
      child: Column(
        children: [
          ...
          Padding(
              ...
              child: Text(
                  "iphone have a water resistance rating of 50 metres under ISO standard 22810:2010. This means that they may However, the For additional information, see support.apple.com/en-in/HT205000. Series 8 is also rated IP6X dust resistant.",
                  style: TextStyle(
                      fontSize: 17,
                      foreground: Paint()
...
class ibuttons extends StatelessWidget {
  ibuttons({Key? key, required this.index, required this.routeargs})
      : super(key: key);
      ...
      children: [
        ElevatedButton.icon(
            ...
            onPressed: () {
             ...
             // flag.final_price = 0;
              // you can also write like this dataprovider().flag
              //dataprovider is nothing but the object we created at main file
              if (flag.flag) {

                add_to_cart(
                    object[routeargs][index].title,
                    object[routeargs][index].imageurl,
                    object[routeargs][index].desc,
                    object[routeargs][index].price,
                    object[routeargs][index].quntity,email);
